...
c v_flory = 0.558 +- 0.019 and turn = 1.062 +- 0.082
c Used to define the PS, ID, and Folded sectors of a turn vs nu plot


      r_model(middle_position)=turn/v_flory
      if(v_flory.ge.(0.558)) then
         classification(middle_position)='D'
         goto 10
      endif
      if(turn.gt.(1.144).and.v_flory.lt.(0.558)) then
         classification(middle_position)='P'
         goto 10
      endif
      if(turn.gt.(1.062).and.v_flory.lt.(0.539)) then
         classification(middle_position)='P'
         goto 10
      endif

      slope=0.019/0.082
      intercept=0.539-slope*1.062
      ...
      if(turn.le.(0.98).and.v_flory.lt.(0.558)) then
         classification(middle_position)='F'
         goto 10
      endif
      if(turn.le.(1.062).and.v_flory.lt.(0.539)) then
         classification(middle_position)='F'
         goto 10
      endif
